...
// HP <= 0 means destroyed; HP == config.grid.surfaceResistance means fully dirty.
// Float32Array so damage is dt-based (frame-rate independent).
...
export function initGrid(W: number, H: number) {
  ...
  grid.data = new Float32Array(grid.cols * grid.rows);
